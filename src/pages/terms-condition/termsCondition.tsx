import AccordionComponent, { type AccordionItemData } from '@/components/ui/accordion/accordion-components';
...
const TermsCondition: React.FC = () => {
  ...
  const items: AccordionItemData[] = [
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col self-stretch text-sm font-poppins leading-6 text-[#262626]">
          {t('termAndCondition.tnc.desc.5').split('\n').map((p: string, i: number) => (
            <p key={`d5-${i}`} className="mb-3">{p}</p>
          ))}
          <ul className="list-disc pl-5 ml-1 mb-3">
            {Object.values(t('termAndCondition.tnc.list.5', { returnObjects: true }) as Record<string, string>)
              .map((item, i) => <li key={`l5-${i}`} className="mb-1">{item}</li>)}
          </ul>
          {t('termAndCondition.tnc.desc.6').split('\n').map((p: string, i: number) => (
            <p key={`d6-${i}`} className="mb-3">{p}</p>
          ))}
        </div>
      ),
    },
